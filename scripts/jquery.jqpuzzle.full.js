...
$.fn.jqPuzzle = function(settings, texts) {
	...
	var OrderedNumber = function($pieces) {
	    totale=0;
	        // iterate over pieces and check each piece
	        for(var i = 0; i < $pieces.length; i++) {
	            // since the hole is not saved in the pieces array, 
	...
	return this.filter('img').each(function(){
		...
		$srcImg.one('load', function() {
			...
			var shuffle = function(rounds, speed) {
				...
				if ($ordered) $ordered.val(ordered);
			...
			var $protoPiece = $('<div/>')
				...
				.append($('<span/></br>')); // will hold the numbers
			...
			var $display, $counter, $timer , $ordered;
			if(control.counter || control.timer || control.ordered) {
			    ...
			    // create ok component
				if (control.ordered) $ordered = $protoField.clone()
					.appendTo($display)
					.after(texts.okLabel);

				//$('<hr><br>').appendTo($display)
		    ...
		    //				    width: fullWidth + 2 * (computedStyles.wrapper.padding + computedStyles.wrapper.border) + boxModelHack.gui.width + 10,
			...
			$pieces.click(function() {
                ...
                // update pezzi in ordine
				if ($ordered) {
				    $ordered.val(OrderedNumber($pieces));
				    $('input')[1].value = (OrderedNumber($pieces));
				    $('#scorenum').text( Math.round(seconds * 100 * OrderedNumber($pieces) / (moves*10)));
				}


				if ((OrderedNumber($pieces) == $pieces.length) && (moves > 0))
				{
				    finishGame();
				}
				...
				if(moves == 1) {
					...
					if(!timer) timer = new Timer(333, function(ms) {
						...
						if ($timer) $timer.val(seconds);
						if ($timer) $('input')[2].value = seconds;
